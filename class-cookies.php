...
use function WeCodeArt\Functions\{ get_prop, toJSON, get_lightness_limit };
...
final class Cookies implements Integration {
			...
			<table class="wp-cookies-table is-style-stripes table-bordered table-hover">
				<thead class="has-accent-background-color">
					<tr>
						...
						<th class="has-text-align-center"><?php esc_html_e( 'Status', 'wecodeart' ); ?></th>
					</tr>
				</thead>
				<tbody class="has-small-font-size"><?php
					...
					?><tr class="wp-cookies-table__item" data-category="<?php echo esc_attr( $category ?: 'other' ); ?>">
						...
						<td 
							...
							data-wp-context="<?php echo esc_attr( toJSON( [
								'container'	=> '#wp-cookies-modal',
								'title'		=> esc_html__( 'This cookie is strictly necessary and cannot be disabled.', 'wecodeart' )
							] ) ); ?>"
	...
	public function display_actions( string $module = 'buttons' ): string {
		...
		<div class="grid my-3" style="--wp--columns: 2">
			<?php
				...
				\wecodeart_input( 'floating', [
					'type'		=> 'search',
					'label'		=> esc_html__( 'Cookie name', 'wecodeart' ),
					'attrs'		=> [
						'placeholder' 	=> ' ',
				...
				\wecodeart_input( 'floating', [
					'type'		=> 'select',
					'label'		=> esc_html__( 'Category', 'wecodeart' ),
					'attrs'		=> [
		...
		<div class="wp-block-buttons justify-content-between">
			...
			<div class="wp-block-button"><?php
				...
				wecodeart_input( 'button', [
					'label' => __( 'Confirm choices', 'wecodeart' ),
					'attrs' => wp_parse_args( [
						'data-value'	=> 'save',
					], $attributes ),
				] );
			...
			<div class="wp-block-button"><?php
				...
				wecodeart_input( 'button', [
					'label' => __( 'Save & Accept all', 'wecodeart' ),
					'attrs' => wp_parse_args( [
		...
		return apply_filters( 'wecodeart/filter/support/cookies/' . $module, ob_get_clean() );
	}
		...
		if( ! is_null( $settings ) ) {
			\add_action( 'wp_loaded', function() use( $settings ) {
				...
				if( wecodeart( 'support' )->has( 'modules/formatting' ) ) {
					$table = wecodeart( 'support' )->get( 'modules/formatting' )::get_instance()->render_block( $table, [
				...
				wecodeart_template( 'general/modal', [
					'id'		=> $settings,
					'classes'	=> $classes,
					'close'		=> get_prop( $this->config, [ 'modal', 'close' ] ),
					'title'		=> apply_filters( 'wecodeart/filter/support/cookies/modal/title', get_prop( $this->config, [ 'modal', 'title' ] ) ),
					'content' 	=> $content,
					'footer'	=> $buttons
				] );
			}, PHP_INT_MAX );
	...
	public function template_area( $default ): array {
		$template_part_area = [
			'area'        => self::CONTEXT,
			'label'       => esc_html__( 'Cookies', 'wecodeart' ),
			'description' => esc_html__( 'The Cookies templates notifies and provides users with access to the cookies manager (soon).', 'wecodeart' ),
			'icon'        => 'footer',
			'area_tag'    => 'div',
		];
	...
	public function get_asset( string $type, string $name ): string {
		...
		$file_path = wecodeart_config( 'paths' )['uri'] . '/inc/support/modules/cookies/assets/' . $file_path . '/' . $type . '/' . $file_name;
	...
	private function get_content( string $slug = '' ) {
		...
		if( count( $saved_templates ) ) {
			...
			$post_id_ = wp_insert_post( [
				...
				'post_type'		=> 'wp_template_part',
				'post_title'	=> esc_html__( join( ' ', array_map( 'ucfirst', explode( '-', $slug ) ) ), 'wecodeart' ),
				'post_name'		=> $slug,
				'post_status'	=> 'publish',
			] );
	...
	private function get_categories(): array {
		return apply_filters( 'wecodeart/filter/support/cookies/categories', [
			'null'		=> esc_html__( 'All', 'wecodeart' ),
			'necessary'	=> esc_html__( 'Necessary', 'wecodeart' ),
			'other'		=> esc_html__( 'Other', 'wecodeart' ),
		] );
	}
	...
	public static function get_defaults(): array {
		return [
			...
			'offcanvas'		=> [
				...
				'keyboard'	=> false,
				'close' 	=> false,
				'title' 	=> esc_html__( 'Cookies', 'wecodeart' ),
				'style'		=> [
			...
			'toast'			=> [
				'enable'	=> true,
				'delay'		=> 5000,
				'confirm'	=> esc_html__( 'Your cookies preferences are saved.', 'wecodeart' ),
				'reject'	=> esc_html__( 'Your rejected cookies - some functionalities might not work as expected.', 'wecodeart' ),
			],
			'toggler'	=> [
				'position'	=> 'left',
				'style'		=> [
